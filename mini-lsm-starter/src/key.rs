...
impl<'a> Key<&'a [u8]> {
    ...
    /// # Safety
    /// 调用者必须确保返回KeyBytes的生命周期不超过Self。   
    /// 只能用于某些接口需要临时KeyBytes，而又不想为了参数拷贝self
    pub unsafe fn to_tmp_key_bytes(self) -> KeyBytes {
        KeyBytes::from_bytes_with_ts(
            Bytes::from_static(std::mem::transmute::<&[u8], &[u8]>(self.key_ref())),
            self.ts(),
        )
    }
